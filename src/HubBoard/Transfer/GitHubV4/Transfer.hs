...
module HubBoard.Transfer.GitHubV4.Transfer where -- todo  and local where
...
getGitHub :: (FromJSON output) => GetMapper output -> IO [output] -- todo rename to get
getGitHub = getGitHubV4 Nothing []

  where
    getGitHubV4 :: Cursor -> [output] -> GetMapper output -> IO [output]
    getGitHubV4 cursor acc mapper@(GetMapper toQuery asHasNext asCursor parse)
        = do
...
fetchLbs
    :: (Owner -> Repository -> Cursor -> String) -> Cursor -> IO LBS.ByteString
